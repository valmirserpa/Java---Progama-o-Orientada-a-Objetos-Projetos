...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println(textoData.format(formatarData));
		     	 ...
		     	 * 
		     	 * Para definir como eu quero formatar a data, posso informar a função .format
		     	 * e passar como parâmetro o objeto DateTimeFormatter*/
		
		
		System.out.println(textoData.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
		
				/*Também é possível instaciar o objeto no momento de rodar a linha*/
		
		
		System.out.println(formatarData.format(textoData));
		
				/* Outra forma, chamar primeiro o formato customizado, depois a função .format e
				 * passar como parâmetro a variárial;
				 * 
				 * */
		...
		System.out.println(formatarDataInstant.format(textoDataHoraFuso));
				...
				/*No caso de horários tempo real (instant), ele não possui a função format como
				 * os demais, neste caso será necessário criar um formato customizado com a informação de fuso horário.
				 * 
				 * Para imprimir na tela, chama primeiro o nome do formato customizado, para invocar o
				 * .format e passar com parâmetro a variável do Instant
				 * */
		...
		/*Também podemos utilizar os modelos de formatação de datas/horas disponíveis no java/*/
